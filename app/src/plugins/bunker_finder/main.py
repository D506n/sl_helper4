...
from .models import FinderParams
...
def build_change_handler(params: FinderParams, pubsub: PubSub, *args, **kwargs):
...
def build_run_string_tool(pubsub: PubSub):
    async def run_string_tool(e: Event):
        params: FinderParams = e.ctx['payload']['params']
...
def _sb():
    with ui.item(on_click=lambda e: ui.navigate.to(URL)) as i:
        ...
        with ui.item_section():
            ui.item_label('Поиск по бункеру')
        return i
